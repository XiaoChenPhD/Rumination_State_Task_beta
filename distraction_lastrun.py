...
expName = 'distraction'  # from the Builder filename that created this script
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_welcome = visual.TextStim(win=win, name='text_welcome',
        text='During the MRI, please focus on the idea expressed by the phrase on the screen and use your imagination to visualize the idea as vividly as possible. Please don’t stop thinking when the MRI is on\n\n\nPlease try your best not to move your head or body\n\n\nPress a button to continue\n\n',
        font='Arial',
    ...
    distraction_loop = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('distraction_prompts.xlsx'),
        seed=None, name='distraction_loop')
    ...
    for thisDistraction_loop in distraction_loop:
        ...
        for thisComponent in distractionComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('distraction.stopped', globalClock.getTime(format='float'))
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
